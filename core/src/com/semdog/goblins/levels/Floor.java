...
public class Floor extends LevelElement {
    ...
    static {
        System.out.println("Made Floor Model");
        baseModel = new ModelBuilder().createBox(10, 1, 10,
                new Material(TextureAttribute.createDiffuse(TextureMaster.get("floor1"))),
                VertexAttributes.Usage.Position | VertexAttributes.Usage.Normal | VertexAttributes.Usage.TextureCoordinates);
    }

    public Floor(Level level, int x, int y) {
        super(level, x, y);

        model = new ModelInstance(baseModel);
        model.transform.setToTranslation(x * 10, 0, y * 10);
    }
